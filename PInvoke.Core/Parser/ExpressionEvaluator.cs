...
namespace Parser
{
	...
	public class ExpressionEvaluator
	{
		...
		public bool TryEvaluate(string expr, 		[Out()]
ref ExpressionValue result)
		{
			List<Token> list = Scanner.TokenizeText(expr, _opts);
			return TryEvaluate(list, ref result);
		}

		public bool TryEvaluate(List<Token> list, 		[Out()]
ref ExpressionValue result)
		{
			...
			return TryEvaluate(node, ref result);
		}

		public bool TryEvaluate(ExpressionNode node, 		[Out()]
ref ExpressionValue result)
		{
		...
		protected virtual bool TryEvaluateLeaf(ExpressionNode node)
		{
			...
			if (token.IsNumber) {
				object value = null;
				if (!TokenHelper.TryConvertToNumber(node.Token, value)) {
					return false;
				}
				...
				char cValue = '0';
				if (!TokenHelper.TryConvertToChar(node.Token, cValue)) {
					return false;
				}
				...
				string sValue = null;
				if (!TokenHelper.TryConvertToString(token, sValue)) {
					return false;
				}
